 ...
 *
 * Time-stamp: <2010-06-29 16:35:54 wbb>
 */
...
public class Semant {
    ...
    public Semant(ErrorMsg.ErrorMsg err, Translate.Level l) {
        this(new Env(err, l), l);
    }

    Semant(Env e, Translate.Level l) {
        env = e;
        level = l;
    }
    ...
    Exp transDec(Absyn.FunctionDec d) {
        for (Absyn.FunctionDec p = d, pre = null; p != null; p = p.next) {
            if (env.venv.isRedefinition(p.name)) {
                ...
                env.venv.put(p.name, new FunEntry(formals, result, level, new Label()));
                pre = p;
            }
        }
        ...
        for (; d != null; d = d.next) {
            ...
            for (Types.RECORD r = formals; r != null; r = r.tail) {
                env.venv.put(r.fieldName, new VarEntry(r.fieldType, level.allocLocal(true)));
            }
            Util.BoolList bl = null;
            for (Absyn.FieldList p = d.params; p != null; p = p.tail) {
                bl = new Util.BoolList(true, bl);
            }
            Semant newsem = new Semant(env, new Translate.Level(level, d.name, bl));
            newsem.transExp(d.body);
            env.venv.endScope();
        }
        return null;
    }
    ...
    Exp transDec(Absyn.VarDec d) {
        if (env.venv.isRedefinition(d.name)) {
            ...
            // Short forma dec
            if (d.typ == null)
                env.venv.put(d.name, new VarEntry(et.ty, level.allocLocal(true)));
            else {  // Long form dec
                Types.Type ty = transTy(d.typ);
                if (et.ty.coerceTo(ty))
                    env.venv.put(d.name, new VarEntry(ty, level.allocLocal(true)));
                else {
                    error(d.pos, "init exp type incompatible");
                    env.venv.put(d.name, new VarEntry(ty.actual(), level.allocLocal(true)));
                }
            }
        }
        return null;
