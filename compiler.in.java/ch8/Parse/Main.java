...
public class Main {
    ...
    public static void main(String argv[]) {
        ...
        ErrorMsg.ErrorMsg errMsg = new ErrorMsg.ErrorMsg(filename);
        Yylex lexer = new Yylex(inp, filename, errMsg);
        ...
        Translate.Translator translator = new Translate.Translator();
        Semant.Semant semant =
            new Semant.Semant(errMsg,
                              new Translate.Level(new Mips.MipsFrame()),
                              translator);
        Translate.Frag frags;
        if (isparse) {
            frags = semant.transProg(absyn);
            if (frags == null) {
                if (semant.nerrors == 1)
