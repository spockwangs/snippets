 ...
 *
 * Time-stamp: <2010-07-02 18:33:39 wbb>
 */
...
public class Semant {
     ...
     * Type-check the exp.
     *
     * @return Translate.Frag linked list if OK, or null on error.
     */
    public Translate.Frag transProg(Absyn.Exp exp) {
        Exp e = transExp(exp, null).exp;
        if (nerrors == 0) {
            ir_tree = translator.irFromExp(e);
    ...
    ExpTy transExp(Absyn.SeqExp e, Label done) {
        ...
        if (et == null) {
            ...
            return new ExpTy(translator.seqExp(tel), et.ty);
        }
    }
        ...
        for (; d != null; d = d.next) {
            ...
            for (int i = 1; r != null; r = r.tail, i++) {
                env.venv.put(r.fieldName,
                             new VarEntry(r.fieldType, level.accessParam(i)));
            }
            ...
            env.venv.endScope();
            el.push(translator.functionDec(newlevel, fun_et.exp));
        }
        if (iserror)
            return null;
        else
            return translator.seqExp(el);
    }
